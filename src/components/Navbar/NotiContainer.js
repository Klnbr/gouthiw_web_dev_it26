...
const NotiContainer = () => {
    ...
    const adminItemPress = async (reportId, notiId) => {
        try {
            const responseDetail = await axios.get(`${apiAddress}/report-detail/trivia/${reportId}`);
            const reportData = responseDetail.data;
            
            if ( reportData.status === null ) {
                const updateStatus = { status: 0 };

                await axios.put(`${apiAddress}/report/${reportId}/status`, updateStatus);
                ...
                await axios.put(`${apiAddress}/report/notifications/${notiId}/read`, { isRead: true, role: "admin" });
            }
    ...
    const handleItemPress = async (reportId, notiId) => {
        try {
            const response = await axios.get(`${apiAddress}/report-detail/trivia/${reportId}`);
            const reportData = response.data;

            await axios.put(`${apiAddress}/report/notifications/${notiId}/read`, { isRead: true, role: "reporter" });
    ...
    const handleItemPressTrivia = async (trivId, notiId) => {
        try {
            const response = await axios.get(`${apiAddress}/trivia/${trivId}`);
            const triviaData = response.data;

            await axios.put(`${apiAddress}/report/notifications/${notiId}/read`, { isRead: true, role: "reported" });
